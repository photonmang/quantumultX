...
function ReadCookie() {
  ...
  const EnvInfo3 = $nobyda.isJSBox ? "JD_Cookie3" : "CookieJD3"
  if (DeleteCookie) {
...
function double() {
  ...
  if (DualKey || $nobyda.read($nobyda.isJSBox ? "JD_Cookie2" : "CookieJD2")) {
    ...
    all()
  } else if (DualKey || $nobyda.read($nobyda.isJSBox ? "JD_Cookie3" : "CookieJD3")) {
    KEY = DualKey ? DualKey : $nobyda.read($nobyda.isJSBox ? "JD_Cookie3" : "CookieJD3")
    all()
  } else {
    $nobyda.time();
    $nobyda.done();
  ...
  try {
    if ($request.headers && $request.url.match(/api\.m\.jd\.com.*=signBean/)) {
      ...
      if (CV.match(/pt_key=.+?;/) && CV.match(/pt_pin=.+?;/)) {
		...
		var AccountThree = CK3 ? CK3.match(/pt_pin=.+?;/) ? CK3.match(/pt_pin=(.+?);/)[1] : null : null
        ...
        } else if (!AccountThree || UserName == AccountTwo) {
          var CookieName = " [账号三] ";
