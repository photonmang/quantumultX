...
使用说明:
1.本脚本仅为打印QQ阅读Cookiek，无其他功能
2.请先确定boxjs已有账号的Cookie，如无账号cookie请先获取账号Cookie.
3.本脚本必须配合本库boxjs使用并设置你需要导出的账号数
...
let K=($.getdata("qqStart")|| 1) - 0; //账号起始值
let Z=$.getdata("qqEnd")|| 1; //账号结束值
let R=K
...
if (!qqreadbdArr[K-1]) {
if (K>R)
    $.msg(jsname+R+'到'+(K-1)+'打印成功'+K+'到'+Z+'打印失败', '⚠️提示：请先获取'+K+'到'+Z+'的Cookie','并在boxjs里设置好账号数')

if (K==R&&Z>R)
    $.msg(jsname+K+'到'+Z+'打印失败', '⚠️提示：请先获取'+K+'到'+Z+'的Cookie','并在boxjs里设置好账号数')


if (Z==R)
    $.msg(jsname+K+'打印失败', '⚠️提示：请先获取'+K+'的Cookie','并在boxjs里设置好账号数')
